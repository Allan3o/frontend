...
document.addEventListener("DOMContentLoaded", function(){
  // Eventos de mudança nos produtos
  const itens = document.querySelectorAll(".item-produto, .qtd-produto");
  itens.forEach(el => el.addEventListener("change", calcularTotal));

  mostrarCarrinho();
  calcularTotal();

  const botaoConfirmar = document.getElementById("confirmarRemover");
  if(botaoConfirmar) botaoConfirmar.addEventListener("click", confirmarRemocao);

  // Carregar depoimentos somente se existir o container
  const containerDepoimentos = document.getElementById("lista-depoimentos");
...
function calcularTotal(){
  ...
  if(campoTotal) campoTotal.innerText = formatarMoeda(total);
}
...
function mostrarCarrinho(){
  ...
  carrinho.forEach((produto,index)=>{
    ...
    tabela.innerHTML += `
      <tr>
        ...
        <td>
          <button class="btn btn-danger btn-sm" onclick="abrirConfirmacao(${index})">Remover</button>
        </td>
      </tr>
    `;
  });
  ...
  if(totalCarrinho) totalCarrinho.innerText = formatarMoeda(total);
}
...
function confirmarRemocao(){
  let carrinho = JSON.parse(localStorage.getItem("carrinho")) || [];
  if(itemParaRemover !== null) carrinho.splice(itemParaRemover,1);
